...
const CardsServicesOthers = ({
  ...
  const Photo = () => {
    ...
    return (
      <Image
        style={styles.photoUser}
        source={require("../images/png/sampleUserImage.png")}
      />
    );
  };
  ...
  return (
    <TouchableOpacity style={styles.boxCard}>
      <View style={styles.photoUserBox}>
        <Image style={styles.photoUser} source={photo} />
        <View style={{ position: "absolute", right: 0, bottom: 5 }}>
          <Image style={styles.photo} source={userIcon}/>
        </View>
      </View>
      <View style={styles.boxDetailsUser}>
        <Text style={styles.textNameUser}>{name}</Text>
        <Text style={styles.textTypeUser}>{service}</Text>
        <View style={styles.boxNotes}>
          <Text style={styles.textNoteUser}>{Number(avaliation).toFixed(2).replace(".",",")}</Text>
